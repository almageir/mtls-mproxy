...
tls_server::tls_server(const std::string& port, tls_options settings, stream_manager_ptr proxy_backend, asynclog::LoggerFactory log_factory)
...
{
    auto new_stream = std::make_shared<tls_server_stream>(stream_manager_, ++stream_id_, ctx_, ssl_ctx_, logger_factory_);
