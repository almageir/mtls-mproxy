...
server::server(const std::string &port, stream_manager_ptr proxy_backend, asynclog::LoggerFactory logger_factory)
    : signals_(ctx_)
    , acceptor_(ctx_)
...
{
    auto new_stream = std::make_shared<tcp_server_stream>(stream_manager_, ++stream_id_, ctx_, logger_factory_);
